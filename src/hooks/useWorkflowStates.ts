...
/**
 * Workflow states for the current workspace.
 *
 * Workspace-level since migration 003 — there is no team parameter, because a status is
 * no longer owned by a team. One row per status name per workspace.
 */
export function useWorkflowStates() {
  const { currentWorkspace } = useApp();
  
  const { data: workflowStates, isLoading, error, refetch } = useQuery({
    queryKey: ['workflow_states', currentWorkspace?.id],
    queryFn: async () => {
